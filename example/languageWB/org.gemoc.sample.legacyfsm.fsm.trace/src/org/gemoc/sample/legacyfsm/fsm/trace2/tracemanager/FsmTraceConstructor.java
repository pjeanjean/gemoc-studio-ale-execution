...
public class FsmTraceConstructor implements ITraceConstructor {
		...
		if (lastState == null) {
			...
			for (Resource r : allResources) {
				for (TreeIterator<EObject> i = r.getAllContents(); i.hasNext();) {
					...
					if (o instanceof org.gemoc.sample.legacyfsm.fsm.NamedElement) {
						org.gemoc.sample.legacyfsm.fsm.NamedElement o_cast = (org.gemoc.sample.legacyfsm.fsm.NamedElement) o;
	...
	private boolean addNewObjectToState(org.gemoc.sample.legacyfsm.fsm.NamedElement o_cast,
			fsmTrace.States.SpecificState newState) {
		boolean added = false;
		if (o_cast instanceof org.gemoc.sample.legacyfsm.fsm.StateMachine) {
			added = addNewObjectToState((org.gemoc.sample.legacyfsm.fsm.StateMachine) o_cast, newState);
		}

		return added;
	}// end addNewObjectToState

	private boolean addNewObjectToState(org.gemoc.sample.legacyfsm.fsm.StateMachine o_cast,
			fsmTrace.States.SpecificState newState) {
		boolean added = false;
		...
		if (!added && !exeToTraced.containsKey(o_cast)) {
			...
			tracedObject.setOriginalObject((org.gemoc.sample.legacyfsm.fsm.StateMachine) o_cast);
			...
			firstValue_consummedString
					.setConsummedString((java.lang.String) modelAccessor.getValue(o_cast, "consummedString"));
			...
			if (modelAccessor.getValue(o_cast, "currentState") != null) {
				firstValue_currentState.setCurrentState(
						(org.gemoc.sample.legacyfsm.fsm.State) modelAccessor.getValue(o_cast, "currentState"));
			} else {
				firstValue_currentState.setCurrentState((org.gemoc.sample.legacyfsm.fsm.State) null);
			}
			...
			firstValue_producedString
					.setProducedString((java.lang.String) modelAccessor.getValue(o_cast, "producedString"));
			...
			firstValue_unprocessedString
					.setUnprocessedString((java.lang.String) modelAccessor.getValue(o_cast, "unprocessedString"));
		...
		if (lastState == null) {
			...
			for (org.eclipse.gemoc.xdsmlframework.api.engine_addon.modelchangelistener.ModelChange modelChange : changes) {
				...
				if (modelChange instanceof org.eclipse.gemoc.xdsmlframework.api.engine_addon.modelchangelistener.NonCollectionFieldModelChange) {
					...
					if (o instanceof org.gemoc.sample.legacyfsm.fsm.StateMachine) {
						org.gemoc.sample.legacyfsm.fsm.StateMachine o_cast = (org.gemoc.sample.legacyfsm.fsm.StateMachine) o;

						if (p.getName().equals("unprocessedString")) {
							...
							java.lang.String value = (java.lang.String) modelAccessor.getValue(o_cast,
									"unprocessedString");
						...
						if (p.getName().equals("producedString")) {
						...
						if (p.getName().equals("currentState")) {
							...
							org.gemoc.sample.legacyfsm.fsm.State value = (org.gemoc.sample.legacyfsm.fsm.State) modelAccessor
									.getValue(o_cast, "currentState");

							newValue.setCurrentState((org.gemoc.sample.legacyfsm.fsm.State) value);

							traced.getStateMachine_currentState_Dimension().getValues().add(newValue);
							newState.getValues().add(newValue);
						} else

						if (p.getName().equals("consummedString")) {
